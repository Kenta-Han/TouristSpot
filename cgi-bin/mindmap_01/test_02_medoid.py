...
## ユーザ入力とDBヘ書き込む
sql_insert = "INSERT INTO mindmap_test(user_id, prefecture, area, start_datetime,history) VALUES(%s,%s,%s,%s,%s);"
...
select_user_review = "SELECT spot_id,wakachi2_text FROM review_wkt2 WHERE spot_id IN {} ORDER BY spot_id ASC;".format(tuple(user_spot_id_list))
...
select_review = "SELECT spot_id,wakachi2_text FROM review_wkt2 WHERE spot_id IN {} ORDER BY spot_id ASC;".format(tuple(spot_id_list))
...
sql_update = "UPDATE mindmap_test SET finish_datetime='{finish}', spot_name='{spot_name}', lat='{lat}', lng='{lng}' where id = {record_id};".format(finish=finish_datetime, spot_name=','.join(name), lat=','.join(lat), lng=','.join(lng), record_id=record_id)
