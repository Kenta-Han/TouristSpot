...
import numpy as np
import mypackage.other_def_cos as myp_other_c
# import pandas as pd
# import json
...
like_history = myp_other_c.Make_History_List(history_list)
print(history_list)
select_user_history = "SELECT id,name,lat,lng,area_id,review,category1 FROM spot_mst WHERE name {} AND review != 0;".format(tuple(like_history))

user_spot_list = myp_other_c.SpotORReview_List(select_user_history)
print("<h4>履歴スポット数：\t{}</h4>".format(len(user_spot_list)))
print("<h4>履歴スポット(Spot_id, Name, Lat, Lng, Area_id, Review, Category1)：</h4>\n{}".format(user_spot_list))
...
user_review_list = myp_other_c.SpotORReview_List(select_user_review)
print("<h4>全スポットのレビューの数：\t{}</h4>".format(len(user_review_list)))

## スポットリスト
spot_list = []
for i in range(len(user_spot_list)):
    spot_list.append([user_spot_list[i][1],user_spot_list[i][6]])

## スポット毎のレビュー
user_review_wkt_group_by = myp_other_c.EverySpot_Review(user_review_list)

##  ユーザ履歴のスポットの全レビュー
user_review_wkt_all = myp_other_c.AllSpot_Review(user_review_list)

## 単語に重み付け(TFIDF)
tfidf_list = []
for i in tqdm(range(len(user_review_wkt_group_by))):
    user_review_wkt_all.insert(0,user_review_wkt_group_by[i])
    user_words = myp_other_c.Tfidf(user_review_wkt_all)
    tfidf_list.append(user_words[0])
    user_words = 0

## 履歴レビューを辞書でリストにまとめる
user_words_all = myp_other_c.Change_To_Dic(tfidf_list)

## 特徴語をスポット毎でまとめてリスト作成
spot_values_list = myp_other_c.spot_values_list(tfidf_list)
...
## 各データはどのクラスタに属しているか
pre_labels,cen = myp_other_c.KM(padding,len(padding))
...
# area_id_list = myp_other_c.Area_id_List(select_area_id)
# print("<h4>エリアIDの数：\t{}</h4>".format(len(area_id_list)))
#
# ## エリア内のレビューありスポットリスト
# select_spot = "SELECT DISTINCT id,name,lat,lng,area_id,review FROM spot_mst WHERE area_id IN {} AND review!=0 ORDER BY category1".format(tuple(area_id_list))
# spot_list = myp_other_c.SpotORReview_List(select_spot)
# print("<h4>エリアスポット数(レビューあり)：\t{}</h4>".format(len(spot_list)))
# print("<h4>エリアスポット(Spot_id, Name, Lat, Lng, Area_id, Review, Category1)，(Areaレビューあり)：</h4>{}".format(spot_list))
#
# ## レビューリスト
# spot_id_list = []
...
# review_list = myp_other_c.SpotORReview_List(select_review)
# print("<h4>全スポットのレビューの数：\t{}</h4>".format(len(review_list)))
#
# ## スポット毎のレビュー
# review_wkt_group_by = myp_other_c.EverySpot_Review(review_list)
#
# ## エリア内のスポットの全レビュー
# review_wkt_all = myp_other_c.AllSpot_Review(review_list)
#
# ## 単語に重み付け(TFIDF)
# review_wkt_all.insert(0,review_wkt_group_by[0])
# words = myp_other_c.Tfidf(review_wkt_all)
#
# ## GoogleMapの表示
# name,lat,lng = [],[],[]
# cnt = 0
