...
#
# sql_update = "UPDATE analogy_deim SET unvis_name_f='{unv_f}', vis_name_f='{vis_f}', cossim_f='{cos_f}', word_f='{word_f}', unvis_lat_f='{lat_f}', unvis_lng_f='{lng_f}' WHERE id = {record_id};".format(unv_f='，'.join(sql_unvis_f), vis_f='，'.join(sql_vis_f), cos_f='，'.join(sql_cossim_f), word_f=sql_word_f, lat_f='，'.join(sql_lat_f), lng_f='，'.join(sql_lng_f), record_id=record_id)
...
sql_update = "UPDATE analogy_deim SET code='{code}', unvis_name='{unv}', vis_name='{vis}', cossim='{cos}', unvis_lat='{lat}', unvis_lng='{lng}', word_random='{word_r}', word_harmonic='{word_h}',syori_finishtime='{finish}' WHERE id = {record_id};".format(code=random, unv='，'.join(sql_unvis), vis='，'.join(sql_vis), cos='，'.join(sql_cossim), lat='，'.join(sql_lat), lng='，'.join(sql_lng), word_r=sql_word_r, word_h=sql_word_h, finish=syori_finishtime, record_id=record_id)
cur.execute(sql_update)
conn.commit()

## レスポンス作成（送信）
myp_res.Response(json_wrandom,json_harmonic,json_random,record_id)
