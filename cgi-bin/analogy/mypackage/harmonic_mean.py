...
def Sort_TFIDF_VtoU_Harmonic(vis_tfidf,unvis_tfidf,vis_spot_name,unvis_spot_name,result):
    ## TFIDFの結果にスポット名を追加
    vis_spot,unvis_spot = [],[]
    ...
    for i in tqdm(range(len(all_spot[0]))):
        ...
        for sw in same_word:
            ...
            if len(all_spot[0][i][vi][0])>1 and re.search(bytesymbols,all_spot[0][i][vi][0])==None:
                 temp.append([all_spot[0][i][vi][0],abs(2/(1/all_spot[0][i][vi][1]+1/all_spot[1][i][un][1]))])
        all.append(temp)
