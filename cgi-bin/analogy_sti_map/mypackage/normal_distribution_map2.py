...
def resp(record_id,unvis,unlat,unlng,vis,vislat,vislng,cos,color,word,sql_unvis,sql_vis,sql_word):
    ...
    sql_insert = "UPDATE analogy_sti SET unvis_name_map2='{unv}',vis_name_map2='{vis}',word_map2='{word}' WHERE id = {record_id};".format(unv='，'.join(sql_unvis),vis='，'.join(sql_vis),word=sql_word,record_id=record_id)
