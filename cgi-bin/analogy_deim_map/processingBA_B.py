...
sql_update = "UPDATE analogy_deim_map SET unvis_name='{unv}', vis_name='{vis}', cossim='{cos}', word='{word}', unvis_lat='{lat}', unvis_lng='{lng}', word='{word}',finish_datetime='{finish}',code='{code}',method='BtoA_B' WHERE id = {record_id};".format(unv='，'.join(sql_unvis), vis='，'.join(sql_vis), cos='，'.join(sql_cossim), word=sql_word, lat='，'.join(sql_lat), lng='，'.join(sql_lng), finish=finish_datetime, record_id=record_id, code=random)
...
print("<p style='text-align:center;color:#FF0000;'>「次へ」行く前に，承認コードと未訪問スポット名をコピーしてCrowdWorksの対応するタスクに貼り付けてください．<br>(見つけたスポット数が0件の時「なし」で回答してください)</p><div class='vector_result'></div><p style='text-align:center;'>「次へ」をクリックしてから，結果が表示されるまで，その間再読込を行わないでください．<br>（表示するまで最大150秒かかる場合があります，150秒過ぎると「Timeout」が表示されます．）</p>")
