...
prefecture1 = form.getvalue('prefecture1') ## 都道府県
area1 = form.getvalue('area1') ## エリア
prefecture2 = form.getvalue('prefecture2') ## 都道府県
area2 = form.getvalue('area2') ## エリア
history = form.getlist("visited")
...
cur.execute(sql_insert,(user_id, prefecture1, area1, start_datetime, history2))
conn.commit()

## ユーザの最新情報を得る
cur.execute("SELECT max(id) FROM analogy_deim_map WHERE user_id='{user}' and prefecture='{pre}';".format(user = user_id, pre = prefecture1))
...
## 未訪問エリアIDリスト
if area1 == "None":
    select_unvisited_area_id = "SELECT DISTINCT id FROM area_mst WHERE area1 LIKE '%{pre}%' AND id < 30435;".format(pre = prefecture1)
    unvisited_area_id_list = myp_other.Area_id_List(select_unvisited_area_id)
else:
    select_unvisited_area_id = "SELECT DISTINCT id FROM area_mst WHERE area1 LIKE '%{pre}%' AND (area2 LIKE '%{area}%' OR area3 LIKE '%{area}%') AND id < 30435;".format(pre = prefecture1, area = area1)
    unvisited_area_id_list = myp_other.Area_id_List(select_unvisited_area_id)
...
sql_update = "UPDATE analogy_deim_map SET unvis_name='{unv}', vis_name='{vis}', cossim='{cos}', word='{word}', unvis_lat='{lat}', unvis_lng='{lng}', word='{word}',finish_datetime='{finish}',code='{code}',method='AtoB_A' WHERE id = {record_id};".format(unv='，'.join(sql_unvis), vis='，'.join(sql_vis), cos='，'.join(sql_cossim), word=sql_word, lat='，'.join(sql_lat), lng='，'.join(sql_lng), finish=finish_datetime, record_id=record_id, code=random)
cur.execute(sql_update)
conn.commit()

if area1 == "None":
    print("<h2>訪問したいスポットエリア："+str(prefecture1) + "</h2>")
else :
    print("<h2>訪問したいスポットエリア："+str(prefecture1) + "--" + str(area1) + "</h2>")
print("<h2>見つけたスポット数：" + str(len(all_json)) + "件<h2>")
if len(all_json) != 0 :
    html_body2 = u"""
        <h2>マップ上のアイコンをクリックすると各スポットの詳細情報（スポット名）が表示されます．<br>検索結果が密集していたり，画面外にもあるかもしれません．<br>ズームイン/アウトしながら見てください．</h2>
        <div id='map'>
            <iframe src='https://www.google.com/maps/embed?pb=!1m16!1m12!1m3!1d13271.045898004142!2d139.69292184601608!3d35.690589482984!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!2m1!1z5bel5a2m6Zmi5aSn5a2m!5e0!3m2!1sja!2sjp!4v1541602248205' width='100%'' height='750' frameborder='0' style='border:0'></iframe>
        </div>
    ...
    """
    print(html_body2)
    # print("<script type='text/javascript'>$(document).ready(function() {initMap("+str(all_json)+")});</script>")
...
print("<p style='text-align:center;color:#FF0000;'>「次へ」行く前に，「承認コード」と「未訪問スポット名」をコピーしてCrowdWorksの対応するタスクに貼り付けてください．<br>(見つけたスポット数が0件の時「なし」で回答してください)</p><div class='vector_result'></div><p style='text-align:center;'>「次へ」をクリックしてから，結果が表示されるまで，その間再読込を行わないでください．<br>（表示するまで最大150秒かかる場合があります，150秒過ぎると「Timeout」が表示されます．）</p>")
print("<form style='text-align:center;' method='post'>")
print("<input type='hidden' id='user_id' name='user_id' value='" + str(user_id) + "' />")
for his in history:
    print("<input type='hidden' class='visited' name='visited' value='" + str(his) + "' />")
print("<input type='hidden' name='prefecture2' id='prefecture2' value='" + str(prefecture2) + "' />")
print("<input type='hidden' name='area2' id='area2' value='" + str(area2) + "' />")
html_body3 = u"""
