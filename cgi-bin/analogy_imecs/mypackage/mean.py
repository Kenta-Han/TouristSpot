...
def Sort_TFIDF_VtoU(vis_tfidf,unvis_tfidf,vis_spot_name,unvis_spot_name,result):
    ## TFIDFの結果にスポット名を追加
    vis_spot,unvis_spot = [],[]
    ...
    for i in tqdm(range(len(all_spot[0]))):
        ...
        for sw in same_word:
            ...
            if len(all_spot[0][i][vi][0])>1 and re.search(bytesymbols,all_spot[0][i][vi][0])==None:
                 temp.append([all_spot[0][i][vi][0],abs((all_spot[0][i][vi][1]+all_spot[1][i][un][1])/2)])
        all.append(temp)
    ...
    for i in tqdm(range(len(all_spot[0]))):
        ...
        for sw in same_word:
            ...
            if len(all_spot[0][i][un][0])>1 and re.search(bytesymbols,all_spot[0][i][un][0])==None:
                 temp.append([all_spot[0][i][un][0],abs((all_spot[0][i][un][1]+all_spot[1][i][vi][1])/2)])
        all.append(temp)
        all[i].sort(key=lambda x:x[1]) ## 昇順ソート(0に近い程が良い)
        # ## 未訪問，既訪問，類似度，単語(最初の10個まで)
