...
<body>
<div id='wrap'>
<header><h1 class='title'>観光スポット検索(RS)</h1></header>
"""
print(html_body)
...
for i in tqdm(range(len(spot_list))) :
    c.execute("select spot,word,tfidf_kantou," + str(season_word[0]) + " from tt_unity_kantou where spot in ('" + str(spot_list[i]) + "')")
    for j in c:
...
spot_kld = []
for i in tqdm(range(len(spot_list))) :
    c.execute("select spot,word,tfidf_kantou," + str(season_word[1]) + " from tk_unity_kantou3 where spot in ('" + str(spot_list[i]) + "')")
    for j in c:
...
print("<form action='review_and_season3.py' method='post'>")
myp_other.Average25_rs(kantou_tfidf_all,season_kld_all,record_id)
