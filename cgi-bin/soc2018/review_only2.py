...
# season_id = int(form.getvalue('season_id')) ##季節
...
for i in tqdm(range(len(spot_list))) :
    c.execute("select spot,word,tfidf_kantou from tt_unity_kantou where spot in ('" + str(spot_list[i]) + "')")
    for j in c:
...
print("<form action='review_only3.py' method='post'>")
myp_other.Top10_review_only(kantou_tfidf_all,record_id)
