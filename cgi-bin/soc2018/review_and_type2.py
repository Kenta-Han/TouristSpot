...
<title>観光スポット(RT)</title>
</head>
<body>
<div id='wrap'>
<header><h1 class='title'>観光スポット検索(RT)</h1></header>
"""
print(html_body)
...
# ====== 選択レビュー(insert) ======
sql_update = "update soc2018 set {column1} ='{r_user1}',{column2}='{r_user2}',{column3}='{r_user3}' where id = {record_id};"
...
for i,column in zip(range(len(review_nouser)),review_column):
    c.execute("update soc2018 set " + column + "='" + review_nouser[i][2] + "' where id=" + str(record_id) + ";")
    connect.commit()
...
for i in tqdm(range(len(spot_list))) :
    c.execute("select spot,word,tfidf_kantou," + str(type_word[0]) + " from tt_unity_kantou where spot in ('" + str(spot_list[i]) + "')")
    for j in c:
...
spot_kld = []
for i in tqdm(range(len(spot_list))) :
    c.execute("select spot,word,tfidf_kantou," + str(type_word[1]) + " from tk_unity_kantou3 where spot in ('" + str(spot_list[i]) + "')")
    for j in c:
        spot_kld.append(list(j))
spot_type_kld = myp_spot.Type2(spot_kld)
## ====== MySQLに問い合わせ〆 ======
...
print("<form action='review_and_type3.py' method='post'>")
myp_other.Average25_rt(kantou_tfidf_all,type_kld_all,record_id)
